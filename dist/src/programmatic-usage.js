...
exports.addCommentThatCodeIsGenerated = exports.getHelpText = exports.ts2typebox = void 0;
const prettier = __importStar(require("prettier"));
const cosmiconfig_1 = require("cosmiconfig");
const package_json_1 = __importDefault(require("../package.json"));
const codeOptions_cjs_1 = __importDefault(require("./codeOptions.cjs"));
const codegen = __importStar(require("@sinclair/typebox-codegen"));
/**
 ...
 * @throws Error
 **/
const ts2typebox = async ({ input, disableAutogenComment, skipTypeCreation, }) => {
    const generatedTs = codegen.TypeScriptToTypeBox.Generate(input);
    // post-processing
    // 1. transformations
    const ts2typeboxCodeOptions = await (0, cosmiconfig_1.cosmiconfig)("ts2typebox").search();
    // TODO: perhaps validate with typebox that these are indeed valid codeOption
    // configs (ts2typebox options)
    const usedCodeOptions = ts2typeboxCodeOptions === null
        ? codeOptions_cjs_1.default
        : ts2typeboxCodeOptions.config;
    const transformedTs = transformValuesAndTypes(generatedTs, usedCodeOptions);
    // 2. formatting
    const explorer = (0, cosmiconfig_1.cosmiconfig)("prettier");
    const searchResult = await explorer.search();
    // TODO: perhaps validate with typebox that these are indeed valid prettier configs
    const prettierConfig = searchResult === null ? {} : searchResult.config;
    const resultFormatted = prettier.format(transformedTs, {
        parser: "typescript",
        ...prettierConfig,
    });
    const resultFiltered = skipTypeCreation === undefined
        ? resultFormatted
        : filterTypes(resultFormatted);
    const result = disableAutogenComment === undefined
        ? exports.addCommentThatCodeIsGenerated.run(resultFiltered)
        : resultFiltered;
    return result;
};
exports.ts2typebox = ts2typebox;
/**
 * Post-processing after successful code generation.
 * Transforms all values and types based on given functions.
 */
const transformValuesAndTypes = (generatedTypes, transformFns) => {
    const tsWithTransformedTypes = generatedTypes.replace(/(type)\s(\w+)/gm, (_match, group1, group2) => {
        return group1 + " " + transformFns.transformTypeName(group2);
    });
    const result = tsWithTransformedTypes.replace(/(typeof|const)\s(\w+)/gm, (_match, group1, group2) => {
 ...
 */
exports.addCommentThatCodeIsGenerated = {
    run: (code) => {
        return `/**
...
//# sourceMappingURL=programmatic-usage.js.map